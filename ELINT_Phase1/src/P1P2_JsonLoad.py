...
from P1P2_Filter import taglist_targetslist_filter, track_keywords_filter, follow_medias_filter
...
def Ultra_JsonLoad(input_str, index_line, DiscussionTree, tag_list, userID_list, variables_dict):
	'''
	input_str: str format json
	DiscussionTree: col.defaultdict() 
	tag_list: list of str; hash tags for set day
	userID_list: list of str; list of target's ID
	'''
	# final flag
	flag_final = False
	...
	if flag_TidTimeAuthor:
		try:
			...
			try: 
				...
				Tweet_OBJ['favourites_count'] = user_favourites
			# extract user count variables
			try: 
				user_listed = user_json['listed_count']
			except ValueError:
				pass
			except KeyError:
				pass
			else:
				Tweet_OBJ['listed_count'] = user_listed
			# extract user count variables
			try: 
				user_statuses = user_json['statuses_count']
			except ValueError:
				pass
			except KeyError:
				pass
			else:
				Tweet_OBJ['statuses_count'] = user_statuses
			# extract user count variables
			try: 
				user_verified = user_json['user_verified']
			except ValueError:
				pass
			except KeyError:
				pass
	...
	if variables_dict['load_type'] == 'Ultra_Geo_':
		flag_final, Tweet_OBJ = taglist_targetslist_filter(Tweet_OBJ=Tweet_OBJ, 
														   tag_list=tag_list, 
														   userID_list=userID_list)

	# 'Track_Keyword_' as ELINT keyword_tracking streamed data file
	if variables_dict['load_type'] == 'Track_Keyword_':
		flag_final, Tweet_OBJ = track_keywords_filter(Tweet_OBJ=Tweet_OBJ, 
													  tag_list=tag_list, 
													  userID_list=userID_list, 
													  variables_dict=variables_dict)

	# 'Follow_MediaS_' as ELINT follow comments on major medias' tweets
	if variables_dict['load_type'] == 'Follow_MediaS_':
		flag_final, Tweet_OBJ = follow_medias_filter(Tweet_OBJ=Tweet_OBJ, 
													 tag_list=tag_list, 
													 userID_list=userID_list, 
													 variables_dict=variables_dict)	

	###########################################################################################################
	return flag_final, Tweet_OBJ, DiscussionTree
