...
def Selected_TweetStack_Init(connection, MySQL_DBkey, pin_time, variables_dict):
	'''
...
def Selected_Tweets_Extract(connection, MySQL_DBkey, pin_time, variables_dict):
	'''
	pin_time: pd.timestamp
	Check if exists, then extract
	
	extract method: from variables_dict['extract_type'] by variables_dict['extract_method']

	return: list of dicts, each dict as a tweet
	could return None is table doesn't exist. 
	'''
	# check variables_dict['extract_type']
	tableName = variables_dict['extract_type']+"TweetStack"+pin_time.strftime('_%Y_%m_%d_%H')
	db_name = MySQL_DBkey['db']
...
WHERE table_schema = '%s' AND table_name = '%s'), 1, 0);"""
	...
	comd_Select_Tweets = """
SELECT tweetID,tweetTime,userID,tweetText,Concern_UserName,MenUserList_text,TagList_text,lang,coordinates
FROM %s;"""
	...
	try:
		with connection.cursor() as cursor:
			...
			for entry in result:
				# post-filter...
				post_filter = False
				if variables_dict['extract_method'][0] in str(entry['Concern_UserName']):
					post_filter = True
				elif len(variables_dict['extract_method']) == 2 and variables_dict['extract_method'][1] in str(entry['Concern_UserName']):
					post_filter = True
				...
				if post_filter == True:
					...
					Tweet_Infor['Concerned_Keywords'] = str(entry['Concern_UserName'])
